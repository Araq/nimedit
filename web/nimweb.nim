...
proc buildWebsite(c: var TConfigData) =
  const
    cmd = "nim rst2html --compileonly $1 -o:$2.temp $2.txt"
  if c.ticker.len > 0:
    try:
      c.ticker = readFile(c.ticker)
    except IOError:
  ...
  for i in 0..c.tabs.len-1:
    ...
    var temp = changeFileExt(file, "temp")
    var content: string
    try:
...
proc main(c: var TConfigData) =
  buildWebsite(c)
  #buildAddDoc(c, "upload")
  #buildDocSamples(c, "upload")
  #buildDoc(c, "upload")
