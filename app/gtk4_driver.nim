...
proc initGtk4Driver*() =
  createWindowHook = gtkCreateWindow
  refreshHook = gtkRefresh
  saveStateHook = gtkSaveState
  restoreStateHook = gtkRestoreState
  setClipRectHook = gtkSetClipRect
  openFontHook = gtkOpenFont
  closeFontHook = gtkCloseFont
  measureTextHook = gtkMeasureText
  drawTextHook = gtkDrawText
  getFontMetricsHook = gtkGetFontMetrics
  fillRectHook = gtkFillRect
  drawLineHook = gtkDrawLine
  drawPointHook = gtkDrawPoint
  setCursorHook = gtkSetCursor
  setWindowTitleHook = gtkSetWindowTitle
  pollEventHook = gtkPollEvent
  waitEventHook = gtkWaitEvent
  getClipboardTextHook = gtkGetClipboardText
  putClipboardTextHook = gtkPutClipboardText
  getModStateHook = gtkGetModState
  getTicksHook = gtkGetTicks
  delayHook = gtkDelay
  startTextInputHook = gtkStartTextInput
  quitRequestHook = gtkQuitRequest
