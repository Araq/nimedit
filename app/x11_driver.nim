...
proc initX11Driver*() =
  # Screen hooks
  createWindowHook = x11CreateWindow
  refreshHook = x11Refresh
  saveStateHook = x11SaveState
  restoreStateHook = x11RestoreState
  setClipRectHook = x11SetClipRect
  openFontHook = x11OpenFont
  closeFontHook = x11CloseFont
  measureTextHook = x11MeasureText
  drawTextHook = x11DrawText
  getFontMetricsHook = x11GetFontMetrics
  fillRectHook = x11FillRect
  drawLineHook = x11DrawLine
  drawPointHook = x11DrawPoint
  setCursorHook = x11SetCursor
  setWindowTitleHook = x11SetWindowTitle
  # Input hooks
  pollEventHook = x11PollEvent
  waitEventHook = x11WaitEvent
  getClipboardTextHook = x11GetClipboardText
  putClipboardTextHook = x11PutClipboardText
  getModStateHook = x11GetModState
  getTicksHook = x11GetTicks
  delayHook = x11Delay
  startTextInputHook = x11StartTextInput
  quitRequestHook = x11QuitRequest
